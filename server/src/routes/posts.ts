...
const getPosts = async(req: Request, res: Response) => {
    ...
    try {
      const posts = await Post.find({
        ...
        relations: ["study", "votes", "comments"],
  ...
  const createPost = async (req: Request, res: Response) => {
    const { title, body, study } = req.body;
    if (title.trim() === "") {
    ...
    try {
      const studyRecord = await Study.findOneByOrFail({ name: study });
      const post = new Post();
      post.title = title;
      post.body = body;
      post.user = user;
      post.study = studyRecord;
